"use client"

import { useState, useEffect } from "react"
import { Button } from "@/components/ui/button"
import { ArrowRight, ChevronLeft, ChevronRight } from "lucide-react"

const heroSlides = [
  {
    image: "/luxurious-modern-living-room-interior-design-with-.jpg",
    title: "Designing spaces that feel like home.",
    subtitle: "We create calm, beautiful, and functional interiors for modern living.",
  },
  {
    image: "/elegant-bedroom-interior-design-with-neutral-tones.jpg",
    title: "Where elegance meets comfort.",
    subtitle: "Transform your bedroom into a peaceful sanctuary of style and rest.",
  },
  {
    image: "/contemporary-kitchen-interior-design-with-marble-c.jpg",
    title: "Kitchens crafted for living.",
    subtitle: "Functional beauty that brings families together every day.",
  },
  {
    image: "/modern-office-interior-design-with-minimalist-furn.jpg",
    title: "Workspaces that inspire.",
    subtitle: "Professional environments designed for productivity and creativity.",
  },
]

export function HeroSection() {
  const [currentSlide, setCurrentSlide] = useState(0)

  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % heroSlides.length)
    }, 5000)
    return () => clearInterval(timer)
  }, [])

  const goToSlide = (index: number) => setCurrentSlide(index)
  const prevSlide = () => setCurrentSlide((prev) => (prev - 1 + heroSlides.length) % heroSlides.length)
  const nextSlide = () => setCurrentSlide((prev) => (prev + 1) % heroSlides.length)

  return (
    <section className="relative h-screen w-full overflow-hidden">
      {/* Background Image Carousel */}
      {heroSlides.map((slide, index) => (
        <div
          key={index}
          className={`absolute inset-0 transition-opacity duration-1000 ${index === currentSlide ? "opacity-100" : "opacity-0"
            }`}
        >
          <img src={slide.image || "/placeholder.svg"} alt={slide.title} className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-[#000000]/20" />
        </div>
      ))}

      <div className="relative z-10 h-full flex items-center justify-center">
        <div className="mx-auto max-w-7xl px-6 lg:px-8 w-full">
          <div className="max-w-3xl mx-auto text-center">
            {heroSlides.map((slide, index) => (
              <div
                key={index}
                className={`transition-all duration-700 ${index === currentSlide ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 absolute"
                  }`}
              >
                {index === currentSlide && (
                  <>
                    <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl xl:text-7xl leading-tight text-white text-balance">
                      {slide.title}
                    </h1>
                    <p className="mt-6 text-lg md:text-xl text-white/90 leading-relaxed max-w-xl mx-auto">
                      {slide.subtitle}
                    </p>
                  </>
                )}
              </div>
            ))}
            <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
              <Button className="bg-[#0d3d3d] text-white hover:bg-[#1a5a5a] rounded-full px-8 py-6 text-sm">
                View Our Work
                <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
              <Button
                variant="outline"
                className="rounded-full px-8 py-6 text-sm border-white text-white hover:bg-[#0d3d3d] hover:text-white bg-transparent"
              >
                Book a Consultation
              </Button>
            </div>
          </div>
        </div>
      </div>

      {/* Navigation Arrows */}
      <button
        onClick={prevSlide}
        className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full flex items-center justify-center bg-white/10 hover:bg-[#0d3d3d] backdrop-blur-sm transition-colors"
      >
        <ChevronLeft className="h-6 w-6 text-white" />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full flex items-center justify-center bg-white/10 hover:bg-[#0d3d3d] backdrop-blur-sm transition-colors"
      >
        <ChevronRight className="h-6 w-6 text-white" />
      </button>

      {/* Slide Indicators */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex gap-3">
        {heroSlides.map((_, index) => (
          <button
            key={index}
            onClick={() => goToSlide(index)}
            className={`h-2 rounded-full transition-all duration-300 ${index === currentSlide ? "w-8 bg-[#0d3d3d]" : "w-2 bg-white/50 hover:bg-white/70"
              }`}
          />
        ))}
      </div>
    </section>
  )
}
